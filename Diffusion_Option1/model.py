#!/usr/bin/env python3
"""
Diffusion Option 1 (A1): conditional DDPM to generate post-scan from pre-scan.

- Uses MONAI Generative DiffusionModelUNet backbone
- Conditioning: concatenate noisy post slice with pre slice (2-channel input)
- Loss: MSE between predicted noise and true noise (epsilon)
- Train/Val/Test split:
    * 20 scans held out as TEST (exactly)
    * remaining used for Train+Val, with 12% of that as internal val

Outputs (all inside this Diffusion_Option1 folder):
    runs_diffusion_post_from_pre/
        best_diffusion_model.pt
        pred_samples/
            in_XXXX.npy  (pre, resized 128x128, [0,1])
            out_XXXX.npy (diffusion prediction)
            tgt_XXXX.npy (post ground truth)
"""
...
# Base directory = this script's folder (Diffusion_Option1)
BASE_DIR = os.path.dirname(os.path.abspath(__file__))
# Root directory where pre_scans / post_scans live (parent)
...
# Diffusion config
NUM_TRAIN_TIMESTEPS = 1000      # DDPM steps for training
NUM_INFER_STEPS = 80            # steps for sampling (smaller for speed)
LR = 1e-4

# Optional: path to a pretrained DiffusionModelUNet checkpoint
# If file exists, we load it and fine-tune on your data.
PRETRAINED_CKPT = None  # e.g. "/path/to/pretrained_diffusion_unet.pt"

# Internal validation fraction (over the train pool)
VAL_FRACTION = 0.12
...
def load_middle_axial_slice(nii_path: str) -> np.ndarray:
    """
    Load NIfTI → take middle axial slice (H,W). If 4D, use first vol.
    """
...
def to_tensor_resized_1chw(x2d: np.ndarray, target_hw=TARGET_SIZE) -> torch.Tensor:
    """
    (H,W) → (1,Ht,Wt) float32, resized via bilinear.
    """
...
def sample_ddpm(model, scheduler, pre_batch, num_steps=NUM_INFER_STEPS):
    ...
    with torch.no_grad():
        for t in scheduler.timesteps:
            # t is a scalar tensor (0-D) or int; make a batch version for the model
            t_int = int(t)  # safe even if t is a 0-D tensor
            t_batch = torch.full((B,), t_int, device=device, dtype=torch.long)

            # Conditional input = noisy post (x) + pre-scan
            model_in = torch.cat([x, pre_batch], dim=1)  # (B,2,H,W)

            # Predict noise εθ(x_t, t, pre)
            noise_pred = model(model_in, t_batch)

            # MONAI DDPMScheduler.step **returns a tuple**: (prev_sample, pred_x0)
...
def train_one_epoch(model, scheduler, loader, optimizer, pad_2d=None):
    ...
    for pre_t, post_t in loader:
        pre_t = pre_t.to(DEVICE)    # (B,1,H,W)
        post_t = post_t.to(DEVICE)  # (B,1,H,W)
        if pad_2d is not None:
            pre_t, post_t = _pad_2d_if_needed(pre_t, post_t, pad_2d)

        B = pre_t.size(0)

        # Timesteps: shape (B,), on same device
        t = torch.randint(
            0,
        ...
        # Add noise to the *post* image
        noise = torch.randn_like(post_t)
        noisy_post = scheduler.add_noise(post_t, noise, t)  # (B,1,H,W)

        # Conditional input: [noisy post, pre] → 2 channels
        model_in = torch.cat([noisy_post, pre_t], dim=1)  # (B,2,H,W)

        # IMPORTANT: pass t as 1D (B,) directly to the model
...
def evaluate(model, scheduler, loader, pad_2d=None):
    """
    Full sampling-based evaluation: for each pre/post pair, run DDPM sampling
    and compute L1, MAE, SSIM, PSNR.
    """
    model.eval()
    ...
    for pre_t, post_t in loader:
        ...
        # Generate predictions via DDPM sampling
...
def main():
    ...
    # ---------- Inference on TEST set, save triplets (Week7: 91x109) ----------
    print("\n=== Inference on TEST set (saving slices) ===")
    if not week7:
        test_ds = AxialSlicePairs(test_pre, target_size=TARGET_SIZE)
        test_loader = DataLoader(
            test_ds, batch_size=BATCH_SIZE, shuffle=False,
    ...
    with torch.no_grad():
        for pre_t, post_t in test_loader:
            ...
            pred = sample_ddpm(model, noise_scheduler, pre_t)  # (B,1,H,W)
